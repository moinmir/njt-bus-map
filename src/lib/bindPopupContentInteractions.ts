interface PopupContentInteractionOptions {
...
export function bindPopupContentInteractions(
  scopeElement: HTMLElement,
  options: PopupContentInteractionOptions,
): void {
  const { defaultRouteKey = null, onDirectionChange, onRouteChange } = options;
  const routePanels = Array.from(scopeElement.querySelectorAll<HTMLElement>(".route-panel[data-route-panel]"));

  if (routePanels.length === 0) {
    bindDirectionToggles(scopeElement, defaultRouteKey, true, onDirectionChange);
    return;
  }

  const routeNavButtons = Array.from(
    scopeElement.querySelectorAll<HTMLButtonElement>(".route-nav[data-route-nav]"),
  );
  const currentRouteIconElement = scopeElement.querySelector<HTMLElement>("[data-route-current-icon]");
  const currentRouteLabelElement = scopeElement.querySelector<HTMLElement>("[data-route-current-label]");
  const currentRouteCountElement = scopeElement.querySelector<HTMLElement>("[data-route-current-count]");
  ...
  const setActiveRouteByIndex = (index: number, emitRouteChange: boolean) => {
    ...
    routePanels.forEach((panel, panelIndex) => {
      const isActive = panelIndex === normalizedIndex;
      panel.classList.toggle("is-active", isActive);
      panel.setAttribute("aria-hidden", isActive ? "false" : "true");
    });
    ...
    const activeDirectionKey = bindDirectionToggles(activePanel, activeRouteKey, true, onDirectionChange);
    ...
    scopeElement.dataset.activeRouteIndex = String(normalizedIndex);
  };
  ...
  for (const button of routeNavButtons) {
    ...
    button.addEventListener("click", () => {
      ...
      const currentIndex = Number.parseInt(scopeElement.dataset.activeRouteIndex ?? "0", 10);
      const safeCurrentIndex = Number.isNaN(currentIndex) ? 0 : currentIndex;
